...
#using fake to speed up development
    ...
    do
        ...
        do
            ...
            if [[ "$LOG_GC" == "yes" ]]; then
                ...
                mkdir $gc_dir
                OTHER_ARGS="$OTHER_ARGS --detailed_gc --gc_log $gc_dir --time 60"
            fi
            if [[ "$USE_PIN" == "yes" ]]; then
                pin_dir=$PIN_BASE/$size/$gc_type
                mkdir $pin_dir
                OTHER_ARGS="$OTHER_ARGS --pin --send_markers_to_pin --pin_log $pin_dir --pintool $pintool --time 300"
            fi
            if [[ "$USE_PAPI" == "yes" ]]; then
                papi_dir=$PAPI_BASE/$size/$gc_type
                mkdir $papi_dir
                OTHER_ARGS="$OTHER_ARGS --papi_monitor --papi_sampling --papi_sampling_rate $SAMPLE_RATE --papi_result $papi_dir --time 60"
                if [[ "$PAPI_NAME" != "empty" ]]; then
                OTHER_ARGS="$OTHER_ARGS --papi_config configs/${PAPI_NAME}.txt"
                fi
            fi

            ./${SCRIPT_LOC} $OTHER_ARGS --${gc_type}
            gcCount=$(( $gcCount + 1 ))
        done
        count=$(( $count + 1 ))
    done
